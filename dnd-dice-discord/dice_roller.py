...
def eval_dice_cluster(dice_cluster:str,force_rolls=None):
    ...
    if dice_cluster.startswith('A'):
        if dice_cluster.startswith('A1'):
            dice_cluster = f"{dice_cluster.replace('A1','A').replace('A','2')}k1"
        elif dice_cluster.startswith('Ad'):
            dice_cluster = f"{dice_cluster.replace('A','2')}k1"
        else:
            dice_cluster = f"{dice_cluster.replace('A','').replace('D','')}"
            if dice_cluster.startswith('d'):
                dice_cluster = '1' + dice_cluster
    elif dice_cluster.startswith('D'):
        if dice_cluster.startswith('D1'):
            dice_cluster = f"{dice_cluster.replace('D1','D').replace('D','2')}k-1"
        elif dice_cluster.startswith('Dd'):
            dice_cluster = f"{dice_cluster.replace('D','2')}k-1"
        else:
            dice_cluster = f"{dice_cluster.replace('D','').replace('A','')}"
            if dice_cluster.startswith('d'):
                dice_cluster = '1' + dice_cluster
    elif dice_cluster.startswith('d'):
        dice_cluster = '1' + dice_cluster
