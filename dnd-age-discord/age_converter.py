...
def convert_age_text(from_race:str=None, to_race:str=None, age:int=None):
    # defaults
    if not from_race:
        from_race = 'human'
    if not to_race:
    ...
    if age <= 0:
        ...
        result_age = convert_race_ages(age,from_race,to_race)
...
def race_age_info(race:str):
    ...
    out_text = f'Members of the {race.lower()} race are considered infants until the age of {infant_text}. {maturation_text} are considered adults at {adult_text} of age. {aging_text}A healthy {race.lower()} can live for around {death_text}.'
    return out_text